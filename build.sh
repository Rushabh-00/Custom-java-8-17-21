...
# --- YOUR SUPERIOR LOGGING FUNCTION ---
LOGFILE=build.log
echo "Starting build for Java $TARGET_VERSION on $TARGET_ARCH"
echo "Full logs will be saved to $LOGFILE"
echo
...
run_with_spinner() {
  ...
  if [ $STATUS -ne 0 ]; then
    ...
    echo "--- Displaying last 200 lines of $LOGFILE ---"
    tail -n 200 "$LOGFILE"
    exit $STATUS
  fi
}
...
# Use your logging function for the final compile step
